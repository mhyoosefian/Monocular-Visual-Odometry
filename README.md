...
This repo contains an implementation of a conventional Monocular Visual Odomtery (MVO) and an enhanced MVO referred to as Fast MVO (FMVO). MVO refers to the process of incrementally estimating the position and orientation of a single camera moving in the 3D space. 
...
To run this algorithm, The first step is to download the MH-01 sequence of the [EuRoC mav dataset](https://projects.asl.ethz.ch/datasets/doku.php?id=kmavvisualinertialdatasets). The downloaded folder "mav0" should be placed next to the "MVO" and "FMVO" folders. Then, the `runMe.m` code in "MVO" folder should be run. The results will be stored, and `plotBothResults.m` could be used to generate the plots shown below.
...
To run this algorithm, The first step is to download the MH-01 sequence of the [EuRoC mav dataset](https://projects.asl.ethz.ch/datasets/doku.php?id=kmavvisualinertialdatasets). The downloaded folder "mav0" should be placed next to the "MVO" and "FMVO" folders. Then, the `runMe.m` code in "FMVO" folder should be run. The results will be stored, and `plotBothResults.m` could be used to generate the plots shown below.
